...
/// [`Instant::now()`] is either a direct counter read or a cached runtime-selected read,
/// depending on target. `Instant` is `Send` and `Sync`, comparable, and hashable. It is not a
/// civil-time or OS timestamp.
///
...
impl Instant {
  ...
  ///
  /// Direct targets compile to a single counter read. Runtime-selected targets add a cached
  /// selected-index load and dispatch before the selected counter read.
  ///
  /// # Example
  ///
  /// ```
