...
mod platform {
  ...
  impl PageWriteGuard {
    ...
    fn protect(&self, protection: c_int) -> Result<(), ()> {
      ...
      // SAFETY: `start` and `len` are page-aligned values returned by `page_span`. `mach_task_self_`
      // names the current process task port, and `VM_PROT_COPY` is the Darwin-supported way to add
      // write permission to a private copy of an executable mapping.
