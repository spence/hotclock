...
Numbers are cross-cell empirical medians measured on 5 platforms (Apple Silicon M1 MBP, AWS Graviton 3, AWS Intel t3.medium, AWS Intel m7i.metal-24xl bare-metal, AWS Lambda x86_64). Per-cell breakdown and methodology in [BENCHMARKS.md](https://github.com/spence/tach/blob/main/BENCHMARKS.md). The `tach::Instant` row is dominated by spin-loop calibration error on cells where CPUID 15h isn't available; on the m7i bare-metal cell where it is, tach drift drops to ~4 ppm — close to `std`.
