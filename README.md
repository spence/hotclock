...
| wasm32 (browser / Node)         | `Performance.now()`        |
...
On `wasm32-unknown-unknown`, `Instant::now()` calls `globalThis.performance.now()` via `wasm-bindgen` so the binding works in both main-thread and Web Worker contexts. Browsers typically clamp the resolution to ~100 microseconds for Spectre mitigation; successive calls within that window return identical values (non-decreasing, not strictly increasing). The `wasm-bindgen` dependency is only pulled in when targeting `wasm32-unknown-unknown` — every native target remains zero-dependency. The WASI (`wasm32-wasip1`, `wasm32-wasip2`, `wasm32-wasip1-threads`) and Emscripten (`wasm32-unknown-emscripten`) targets continue to use `std::time::Instant`, which is already backed by `clock_time_get(MONOTONIC)` / POSIX `clock_gettime` — no faster alternative is exposed by those platforms.

The conversion factor from ticks to nanoseconds is read once at first use: from `cntfrq_el0` on aarch64, from `mach_timebase_info` on non-aarch64 macOS, from `QueryPerformanceFrequency` on non-aarch64 Windows, fixed at 1 GHz on `wasm32-unknown-unknown` (`Performance.now()` already returns time units), or via a one-time calibration loop on non-aarch64 Linux.
